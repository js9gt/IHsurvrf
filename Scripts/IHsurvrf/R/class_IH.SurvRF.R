...
setMethod(f = ".PredictAll",
          ...
          definition = function(object, ..., newdata, model, txLevels, txName, params) {
            ...
            ## NOTE: .optimal function same for pooled vs stratified analysis
...
#-------------------------------------------------------------------------------
# For stratified analyses outputting objects of class "SurvRFStratified" from class_DTRSurvStep.R
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
...
## defines a new S4 class called "SurvRFStratified"

setClass(Class = "SurvRFStratified",

         ## 1 slot: strat which is a list
         ## The results of the tree building algorithm for
         ##   each tx group. List will be a list of SurvRF objects

         slots = c("strat" = "list"),

         ## Indicates that SurvRFStratified inherits from SurvRFObject (virtual class)
...
## defines a method for the .Predict() function for class SurfRFStratified objects and NULL newdata

setMethod(f = ".Predict",

          ## method performs actions on objects of class "SurvRFStratified" and when newdata is NULL
          ...
          definition = function(object, newdata, ...) {
            ...
            ## iterate through each of the strata using the @strat slot of the SurfRFStratified object
            for (i in 1L:length(x = object@strat)) {

              ## calls the .Predict() method for each object in the @strat list of class SurvRF objects
              ## since newdata is NULL, retrieves fitted values
              ## we use the .Predict() function written earlier for objects of class SurvRF

              res[[ i ]] <- .Predict(object@strat[[ i ]], newdata = NULL, ...)
            }

            ## returns a list object each element a list containing survFunc, mean,
            ##   and? survProb (ensemble averaged across all trees)
            ## different list for each strata
...
## defines a new method for the .Predict() function to handle objects of class SurvRFStratified when newdata is a data.frame
## method designed to make predictions for new data based on stratified rsf model

## difference between using newdata = NULL vs a dataframe is that for NULL, we just retrieve
## the fitted values from the mode
## when newdata = data.frame, we apply the stratified model to new observations to generate predictions for unseen data
...
setMethod(f = ".Predict",
          ...
          definition = function(object, newdata, ..., params) {
            ...
            ## initializing empty list to store predictions for each stratum

            res <- list()

            ## starting with the first stratum, make predictions
            ## this uses the .Predict() function operating on objects of class SurvRF (defined earlier in script)

            res[[ 1L ]] <- .Predict(object = object@strat[[ 1L ]],
                                    newdata = newdata,
                                    params = params, ...)

            ## looping over remaining stratums, make prediction
            ...
            ## return each stratum's predictions: list where each element corresponds to stratum prediction
            return( res )
...
#-------------------------------------------------------------------------------
# method makes predictions for new data: for objects of class SurvRFStratified
#-------------------------------------------------------------------------------
# AKA when we use stratified analysis in class_DTRSurvStep.R
#-------------------------------------------------------------------------------
# method returns a list object each element a list containing survFunc, mean,
#   and? survProb
#-------------------------------------------------------------------------------
...
## defines a method for the .PredictAll() function for objects of class SurvRFStratified class

setMethod(f = ".PredictAll",
          signature = c(object = "SurvRFStratified"),
          definition = function(object, ..., newdata, model, params, txLevels) {

            ## create a model frame using the input formual in "model" ad the "newdata"
            ## prepares data for prediction by ensuring it matches structure expected by model

            # extract new model frame
            x <- stats::model.frame(formula = model, data = newdata)

            ## if the model formula includes a response variable, remove it
            ## this is important since the response variable isn't needed for prediction, only the predictors
            ...
            ## performs prediction using the first stratum's SurvRF object

            res <- .Predict(object = object@strat[[ 1L ]],
                            newdata = x,
                            params = params, ...)

            ## convert the survFunc into a list to accommodate predictions from later strata

            res$survFunc <- list(res$survFunc)

            ## loop over remaining strata

            i <- 2L
            while (i <= length(x = object@strat)) {

              ## for each strata, make predictions for the respective SurvRF object

              tt <- .Predict(object = object@strat[[ i ]],
                             newdata = x,
                             params = params, ...)

              ## then append the resulting prediction survFunc to the list of survFunc

              res[[ "survFunc" ]][[ i ]] <- tt$survFunc

              ## append the mean survival times and survival probabilities
              ## aka build matrix where each column corresponds to predictions from a different stratum
            ...
            ## determine optimal treatment (line 605)
            ## defined later in this script that returns the optimal treatment, outcome, etc.
            ## NOTE: .optimal function same for pooled vs stratified analysis

            opt <- .optimal(params = params, predicted = res, txLevels = txLevels)

            ## return a list containing aggregated predictions ("res") for each stratum and optimal treatments
