...
#' #' @include class_IH.CriticalValue.R class_IH.CriticalValueMean.R
#' #' @include class_IH.CriticalValueSurvival.R
#' #' @include class_IH.SurvRF.R


source("class_IH.CriticalValue.R")
source("class_IH.CriticalValueMean.R")
source("class_IH.CriticalValueSurvival.R")
source("class_IH.Parameters.R")
source("class_IH.SurvRF.R")
...
.survRF <- function(...,
  ...
  for (i in 1L:nTree) {
    ...
    temp <- .Fortran(
      ...
      nodes = as.double(x = numeric(length = treeDims$nc *
                                      treeDims$nr)),
      ## number of rows * timepoints
      survFunc = as.double(x = numeric(length = treeDims$nr *
                                         nTimes)),
