...
#  }
  ...
  variables <- unique(sub("_(\\d+)", "", setdiff(colnames(data), c("subject.id", "rep.id"))))
  ...
  ### retrieve the survival curves for the predicted survival function for each patient following the optimal treatment
  ## this is in t(pool1_results@optimal@optimalY)-- rows =# of timepoints, columns for each patient and each stage

  ## params@timePoints: contains all the consecutive time points that each of the survival probabilities are calculated at
  ...
  survprob_integral <- matrix(NA, nrow = length(1:(nDP-2)), ncol = nrow(data))
  survprob_integral[1, ] <- mean_values
  ...
  x_append1 <- stats::model.frame(formula = models,
                          ...
                          data = long_data[long_data$stage == (nDP -3), colnames(long_data)[!grepl("^A\\.", colnames(long_data))]],
                          na.action = na.pass)
  ...
  ## get the final stage's area under the curve:
  ## the result after appyling the area function to each column of the matrix of survival probabilities
