...
png("~/Desktop/survrf/Figures/timeline_10pts.png", units = "px", res = 75)
...
p1_save <- ggplot(p1, aes(x = event.time, y = patient, color = factor(stage))) +
  ...
  geom_hline(yintercept = 1:max(p1$patient), linetype = "dashed", color = "gray") +  # Add horizontal lines
...
p2 <- data.frame(
  stage = rep(1:max_stages, each = nrow(rate_failure)),  # Create a column for stages
...
png("~/Desktop/survrf/Figures/avg_rates_10pts.png", units = "px", res = 75)
...
p3 <- data.frame(
  stage = rep(1:max_stages, each = nrow(at_risk)),  # Create a column for stages
  at_risk = c(at_risk)
)

# Calculate the percentage of patients at risk for each stage
p3_percentage <- p3 %>%
  group_by(stage) %>%
  summarize(
...
png("~/Desktop/survrf/Figures/pct_atrisk_10pts.png", units = "px", res = 75)
...
p4 <- data.frame(
  stage = rep(1:max_stages, each = nrow(state_vals)),  # Create a column for stages
...
png("~/Desktop/survrf/Figures/avg_states_10pts.png", units = "px", res = 75)
...
p5 <- data.frame(
  stage = rep(1:max_stages, each = nrow(action_vals)),  # Create a column for stages
...
png("~/Desktop/survrf/Figures/pct_action_10pts.png", units = "px", res = 75)
...
p6 <- data.frame(
  stage = rep(1:max_stages, each = nrow(gamma)),  # Create a column for stages
  gamma = c(gamma),
  delta = c(delta)
)

# Calculate the percentage of patients with gamma = 1 (subject fails before next stage) 0 (not fail)
# calculate the percentage of patients with delta = 0 (censored) , 1 = (not censored)
p6_percentage <- p6 %>%
  group_by(stage) %>% 
...
png("~/Desktop/survrf/Figures/pct_gamma_delta_10pts.png", units = "px", res = 75)

## NOTE: gamma is NA when a patient is censored
...
# Extracting the necessary information for summary statistics
event_time_vals <- pts[, "event.time", , drop = FALSE]

# Reshape the array to long format for easier processing with dplyr
event_time_long <- as.data.frame(as.table(event_time_vals))

# Calculate summary statistics for each stage using dplyr
summary_table <- event_time_long %>%
  
  ## grouping by the stage
  group_by(Var3) %>%
  summarise(
...
write_xlsx(summary_table, path = "~/Desktop/survrf/Outputs/event.time_summary.xlsx")
